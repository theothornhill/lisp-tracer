...
(deftest group-testing
  (testing "Creating a new group"
    (let ((group (make-group)))
      (ok (equal? (shape-transform group) (identity-matrix)))
      (ok (null (group-items group)))))
  (testing "A shape has a parent attribute"
    (let ((s (make-sphere)))
      (ng (shape-parent s))))
  (testing "Adding a child to a group"
    (let ((g (make-group))
      ...
      (add-child g s)
      (ok (group-items g))
      (ok (find s (group-items g) :test 'equalp))
      (ok (equalp (shape-parent s) g))))
  (testing "Intersecting a ray with an empty group"
    (let* ((g (make-group))
           (r (make-ray :origin (make-point :x 0.0 :y 0.0 :z 0.0)
                        :direction (make-vec :x 0.0 :y 0.0 :z 1.0)))
           (xs (local-intersect g r)))
      (ng xs)))
  (testing "Intersecting a ray with an nonempty group"
    (let ((g (make-group))
          (s1 (make-sphere))
          (s2 (make-sphere :transform (translation 0.0 0.0 -3.0)))
          (s3 (make-sphere :transform (translation 5.0 0.0 0.0))))
      (add-child g s1)
      (add-child g s2)
      (add-child g s3)
      (let* ((r (make-ray :origin (make-point :x 0.0 :y 0.0 :z -5.0)
        ...
        (ok (equalp (rt-intersection-object (car xs)) s2))
        (ok (equalp (rt-intersection-object (cadr xs)) s2))
        (ok (equalp (rt-intersection-object (caddr xs)) s1))
        (ok (equalp (rt-intersection-object (cadddr xs)) s1)))))
  (testing "Intersecting a transformed group"
    (let ((g (make-group :transform (scaling 2.0 2.0 2.0)))
          (s (make-sphere :transform (translation 5.0 0.0 0.0))))
      (add-child g s)
      (let* ((r (make-ray :origin (make-point :x 10.0 :y 0.0 :z -10.0)
                          :direction (make-vec :x 0.0 :y 0.0 :z 1.0)))
             (xs (intersect g r)))
        (ok (equal? (length xs) 2)))))
  (testing "Converting a point from world to object space"
    (let ((g1 (make-group :transform (rotation-y (/ pi 2.0))))
          (g2 (make-group :transform (scaling 2.0 2.0 2.0)))
          (s (make-sphere :transform (translation 5.0 0.0 0.0))))
      (add-child g1 g2)
      (add-child g2 s)
