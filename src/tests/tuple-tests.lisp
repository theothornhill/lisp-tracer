...
(deftest tuple-math
  ...
  (testing "Subtracting two points"
    (let ((p1 (make-point 3.0 2.0 1.0))
          (p2 (make-point 5.0 6.0 7.0)))
      (ok (equal? (sub p1 p2) (make-vec -2.0 -4.0 -6.0)))))
  (testing "Subtracting a vector from a point"
    (let ((p (make-point 3.0 2.0 1.0))
          (v (make-vec 5.0 6.0 7.0)))
      (ok (equal? (sub p v) (make-point -2.0 -4.0 -6.0)))))
  (testing "Subtracting two vectors"
    (let ((v1 (make-vec 3.0 2.0 1.0))
          (v2 (make-vec 5.0 6.0 7.0)))
      (ok (equal? (sub v1 v2) (make-vec -2.0 -4.0 -6.0)))))
  (testing "Subtracting a vector from zero vector"
    (let ((zero (zerovec))
          (v (make-vec 1.0 -2.0 3.0)))
      (ok (equal? (sub zero v) (make-vec -1.0 2.0 -3.0)))))
...
(deftest normalizing
  (testing "Normalizing (vec 4 0 0) gives (1 0 0)"
    (let ((v (make-vec 4.0 0.0 0.0)))
      (ok (equal? (normalize v) (make-vec 1.0 0.0 0.0)))))
...
(deftest cross-product
  (testing "The cross product of two vectors"
    (let ((a (make-vec 1.0 2.0 3.0))
          (b (make-vec 2.0 3.0 4.0)))
      (ok (equal? (cross a b) (make-vec -1.0 2.0 -1.0)))
      (ok (equal? (cross b a) (make-vec 1.0 -2.0 1.0))))))

(deftest reflecting
  (testing "Reflecting a vector approaching at 45 degrees"
    (let* ((v (make-vec 1.0 -1.0 0.0))
           (n (make-vec 0.0 1.0 0.0))
           (r (reflect v n)))
      (ok (equal? r (make-vec 1.0 1.0 0.0)))))
  (testing "Reflecting a vector off a slanted surface"
    (let* ((v (make-vec 0.0 -1.0 0.0))
           (n (make-vec (/ (sqrt 2) 2.0)
                        (/ (sqrt 2) 2.0)
                        0.0))
           (r (reflect v n)))
      (ok (equal? r (make-vec 1.0 0.0 0.0))))))
